...
export default function StatusBar(props) {
  ...
  switch (action) {
    ...
    case "chancellor discard":
      ...
      break;
    case "veto requested":
      description = `Chancellor ${chancellorName} has requested to veto!`;
      break;
    case "your veto requested":
      description = `Will you veto?`;
      break;
    case "veto accepted":
      description = `President ${presidentName} has accepted the veto.`;
      break;
    case "veto denied":
      description = `President ${presidentName} has denied the veto.`;
      break;
  ...
  return (
    <div className="status-bar">
      <div className="status-div">
        {fullEvent && <h2 className="status">{description.toUpperCase()}</h2>}
      </div>
    </div>
  );
}
