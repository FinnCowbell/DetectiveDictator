...
export default class SingleInputForm extends React.Component{
  ...
  render(){
    return(
      <div ref="input" className={`single-input-form ${this.props.className || ''}`}>
        {this.props.children}
        <input className="input-box" value={this.state.text} onKeyDown={this.handleSubmit} onChange={this.handleChange}/>
        {this.props.button && 
